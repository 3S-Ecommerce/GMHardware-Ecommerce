...
export class NovoCategoria implements OnInit {
  ...
  // Signal para listar as categorias na tabela embaixo
  ...
  // Busca as categorias do seu back-end na API do Render
  carregarCategorias() {
  ...
  // Método para excluir a categoria chamando a API do seu service
  deletarCategoria(id: string, nome: string) {
    if (confirm(`Tem certeza que deseja excluir a categoria "${nome}"?`)) {
      // Nota: Verifique se o método deleteCategory existe no seu service,
      // caso não exista, veja a observação abaixo do código.
